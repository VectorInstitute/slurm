...
extern List slurm_copy_char_list(List char_list) {
    ...
    if (!char_list || !list_count(char_list)) return NULL;
...
extern int slurm_find_char_in_list(void *x, void *key) {
    ...
    if (!xstrcasecmp(char1, char2)) return 1;
            ...
            if (quote && (names[i] == quote_c))
                ...
                if (!brack_not) {
                    ...
                    if (i != start) {
                        ...
                        if (list_find(itr, slurm_find_char_in_list, name)) {
                            ...
                            count++;
                        if (lower_case_normalization) xstrtolower(name);
                ...
                if ((host_list = hostlist_create(name))) {
                    while ((tmp_this_node_name = hostlist_shift(host_list))) {
                         ...
                         * with QOS.
                         */
                        if (list_find(itr, slurm_find_char_in_list,
                                      this_node_name)) {
                            list_delete_item(itr);
                        } else
                            count++;
        ...
        while (names[i]) {
            ...
            if (quote && names[i] == quote_c)
                ...
                if ((i - start) > 0) {
                    ...
                    if (!name) return 0;
                    while ((tmp_char = list_next(itr))) {
                        if (!xstrcasecmp(tmp_char, name)) break;
                    }
                ...
                if (!names[i]) {
                    info(
                        "There is a problem with your request.  It appears you "
    ...
    while (names[i]) {
        if (quote && names[i] == quote_c)
            ...
            if ((i - start) > 0) {
                ...
                while ((tmp_char = list_next(itr))) {
                    if (!strcasecmp(tmp_char, m_name)) break;
                }
                list_iterator_reset(itr);
...
static int _addto_step_list_internal(List step_list, char *names, int start,
                                     int end) {
    int count = 0;
    char *name;
    ...
    if ((dot = xstrstr(name, "."))) {
        *dot++ = 0;
        ...
        if (plus) {
            /* het step */
            plus++;
            selected_step->step_id.step_het_comp = slurm_atoul(plus);
        }
    } else {
...
slurm_copy_resource_allocation_response_msg(
    ...
    if (msg->cpus_per_node) {
        new->cpus_per_node =
            xcalloc(new->num_cpu_groups, sizeof(*new->cpus_per_node));
    ...
    if (msg->cpu_count_reps) {
        new->cpu_count_reps =
            xcalloc(new->num_cpu_groups, sizeof(*new->cpu_count_reps));
...
extern void slurm_free_last_update_msg(last_update_msg_t *msg) { xfree(msg); }

extern void slurm_init_reboot_msg(reboot_msg_t *msg, bool clear) {
    xassert(msg);

    if (clear) memset(msg, 0, sizeof(reboot_msg_t));
...
extern void slurm_free_shutdown_msg(shutdown_msg_t *msg) { xfree(msg); }
...
extern void slurm_free_return_code_msg(return_code_msg_t *msg) { xfree(msg); }
...
extern void slurm_free_batch_script_msg(char *msg) { xfree(msg); }

extern void slurm_free_job_id_msg(job_id_msg_t *msg) { xfree(msg); }

extern void slurm_free_job_user_id_msg(job_user_id_msg_t *msg) { xfree(msg); }

extern void slurm_free_step_id(slurm_step_id_t *msg) { xfree(msg); }
...
extern void slurm_free_job_desc_msg(job_desc_msg_t *msg) {
    ...
    if (msg) {
        ...
        if (msg->argv) {
            for (i = 0; i < msg->argc; i++) xfree(msg->argv[i]);
        }
        xfree(msg->argv);
...
extern void slurm_free_sib_msg(sib_msg_t *msg) {
    if (msg) {
        ...
        xfree(msg->resp_host);
        if (msg->data) slurm_free_msg_data(msg->data_type, msg->data);
        xfree(msg);
    }
}
...
extern void slurm_free_job_launch_msg(batch_job_launch_msg_t *msg) {
    ...
    if (msg) {
        ...
        slurm_cred_destroy(msg->cred);
        if (msg->environment) {
            for (i = 0; i < msg->envc; i++) xfree(msg->environment[i]);
...
extern void slurm_free_job_info_members(job_info_t *job) {
    ...
    if (job) {
        ...
        if (job->array_bitmap) bit_free((bitstr_t *)job->array_bitmap);
...
extern void slurm_free_node_registration_status_msg(
    ...
    if (msg) {
        ...
        if (msg->energy) acct_gather_energy_destroy(msg->energy);
        xfree(msg->features_active);
        xfree(msg->features_avail);
        if (msg->gres_info) free_buf(msg->gres_info);
        xfree(msg->node_name);
        xfree(msg->os);
        xfree(msg->step_id);
...
extern void slurm_free_resv_desc_msg_part(resv_desc_msg_t *msg,
    ...
    if (res_free_flags & RESV_FREE_STR_USER) xfree(msg->users);
    if (res_free_flags & RESV_FREE_STR_ACCT) xfree(msg->accounts);
    if (res_free_flags & RESV_FREE_STR_TRES_BB) xfree(msg->burst_buffer);
    if (res_free_flags & RESV_FREE_STR_TRES_CORE) xfree(msg->core_cnt);
    if (res_free_flags & RESV_FREE_STR_TRES_LIC) xfree(msg->licenses);
    if (res_free_flags & RESV_FREE_STR_TRES_NODE) xfree(msg->node_cnt);
    if (res_free_flags & RESV_FREE_STR_GROUP) xfree(msg->groups);
}
...
extern void slurm_free_launch_tasks_request_msg(
    ...
    if (msg->het_job_nnodes != NO_VAL) {
        for (i = 0; i < msg->het_job_nnodes; i++) xfree(msg->het_job_tids[i]);
    ...
    if (msg->switch_job) switch_g_free_jobinfo(msg->switch_job);

    if (msg->options) job_options_destroy(msg->options);

    if (msg->select_jobinfo) select_g_select_jobinfo_free(msg->select_jobinfo);
...
extern void slurm_free_signal_tasks_msg(signal_tasks_msg_t *msg) { xfree(msg); }
...
extern void slurm_free_srun_ping_msg(srun_ping_msg_t *msg) { xfree(msg); }
...
extern void slurm_free_srun_timeout_msg(srun_timeout_msg_t *msg) { xfree(msg); }
...
extern enum job_state_reason job_reason_num(char *reason) {
    if (!xstrcasecmp(reason, "None")) return WAIT_NO_REASON;
    if (!xstrcasecmp(reason, "Prolog")) return WAIT_PROLOG;
    if (!xstrcasecmp(reason, "Priority")) return WAIT_PRIORITY;
    if (!xstrcasecmp(reason, "Dependency")) return WAIT_DEPENDENCY;
    if (!xstrcasecmp(reason, "Resources")) return WAIT_RESOURCES;
    if (!xstrcasecmp(reason, "PartitionNodeLimit")) return WAIT_PART_NODE_LIMIT;
    if (!xstrcasecmp(reason, "PartitionTimeLimit")) return WAIT_PART_TIME_LIMIT;
    if (!xstrcasecmp(reason, "PartitionDown")) return WAIT_PART_DOWN;
    if (!xstrcasecmp(reason, "PartitionInactive")) return WAIT_PART_INACTIVE;
    if (!xstrcasecmp(reason, "JobHeldAdmin")) return WAIT_HELD;
    if (!xstrcasecmp(reason, "JobHeldUser")) return WAIT_HELD_USER;
    if (!xstrcasecmp(reason, "BeginTime")) return WAIT_TIME;
    if (!xstrcasecmp(reason, "Licenses")) return WAIT_LICENSES;
    ...
    if (!xstrcasecmp(reason, "Reservation")) return WAIT_RESERVATION;
    if (!xstrcasecmp(reason, "ReqNodeNotAvail")) return WAIT_NODE_NOT_AVAIL;
    if (!xstrcasecmp(reason, "FrontEndDown")) return WAIT_FRONT_END;
    if (!xstrcasecmp(reason, "PartitionDown")) return FAIL_DOWN_PARTITION;
    if (!xstrcasecmp(reason, "NodeDown")) return FAIL_DOWN_NODE;
    if (!xstrcasecmp(reason, "BadConstraints")) return FAIL_BAD_CONSTRAINTS;
    if (!xstrcasecmp(reason, "SystemFailure")) return FAIL_SYSTEM;
    if (!xstrcasecmp(reason, "JobLaunchFailure")) return FAIL_LAUNCH;
    if (!xstrcasecmp(reason, "NonZeroExitCode")) return FAIL_EXIT_CODE;
    if (!xstrcasecmp(reason, "TimeLimit")) return FAIL_TIMEOUT;
    if (!xstrcasecmp(reason, "InactiveLimit")) return FAIL_INACTIVE_LIMIT;
    if (!xstrcasecmp(reason, "InvalidAccount")) return FAIL_ACCOUNT;
    if (!xstrcasecmp(reason, "InvalidQOS")) return FAIL_QOS;
    if (!xstrcasecmp(reason, "QOSUsageThreshold")) return WAIT_QOS_THRES;
    if (!xstrcasecmp(reason, "QOSJobLimit")) return WAIT_QOS_JOB_LIMIT;
    if (!xstrcasecmp(reason, "QOSResourceLimit"))
        return WAIT_QOS_RESOURCE_LIMIT;
    if (!xstrcasecmp(reason, "QOSTimeLimit")) return WAIT_QOS_TIME_LIMIT;
    if (!xstrcasecmp(reason, "BlockMaxError")) return WAIT_BLOCK_MAX_ERR;
    if (!xstrcasecmp(reason, "BlockFreeAction")) return WAIT_BLOCK_D_ACTION;
    if (!xstrcasecmp(reason, "Cleaning")) return WAIT_CLEANING;
    if (!xstrcasecmp(reason, "QOSNotAllowed")) return WAIT_QOS;
    if (!xstrcasecmp(reason, "AccountNotAllowed")) return WAIT_ACCOUNT;
    if (!xstrcasecmp(reason, "DependencyNeverSatisfied"))
        return WAIT_DEP_INVALID;
    if (!xstrcasecmp(reason, "QOSGrpCpuLimit")) return WAIT_QOS_GRP_CPU;
    ...
    if (!xstrcasecmp(reason, "QOSGrpJobsLimit")) return WAIT_QOS_GRP_JOB;
    if (!xstrcasecmp(reason, "QOSGrpMemLimit")) return WAIT_QOS_GRP_MEM;
    if (!xstrcasecmp(reason, "QOSGrpNodeLimit")) return WAIT_QOS_GRP_NODE;
    if (!xstrcasecmp(reason, "QOSGrpSubmitJobsLimit"))
        return WAIT_QOS_GRP_SUB_JOB;
    if (!xstrcasecmp(reason, "QOSGrpWallLimit")) return WAIT_QOS_GRP_WALL;
    ...
    if (!xstrcasecmp(reason, "QOSMinCpuNotSatisfied")) return WAIT_QOS_MIN_CPU;
    if (!xstrcasecmp(reason, "AssocGrpCpuLimit")) return WAIT_ASSOC_GRP_CPU;
    ...
    if (!xstrcasecmp(reason, "AssocGrpJobsLimit")) return WAIT_ASSOC_GRP_JOB;
    if (!xstrcasecmp(reason, "AssocGrpMemLimit")) return WAIT_ASSOC_GRP_MEM;
    if (!xstrcasecmp(reason, "AssocGrpNodeLimit")) return WAIT_ASSOC_GRP_NODE;
    if (!xstrcasecmp(reason, "AssocGrpSubmitJobsLimit"))
        return WAIT_ASSOC_GRP_SUB_JOB;
    if (!xstrcasecmp(reason, "AssocGrpWallLimit")) return WAIT_ASSOC_GRP_WALL;
    if (!xstrcasecmp(reason, "AssocMaxJobsLimit")) return WAIT_ASSOC_MAX_JOBS;
    ...
    if (!xstrcasecmp(reason, "JobHoldMaxRequeue")) return WAIT_MAX_REQUEUE;
    if (!xstrcasecmp(reason, "JobArrayTaskLimit")) return WAIT_ARRAY_TASK_LIMIT;
    ...
    if (!xstrcasecmp(reason, "PowerNotAvail")) return WAIT_POWER_NOT_AVAIL;
    if (!xstrcasecmp(reason, "PowerReserved")) return WAIT_POWER_RESERVED;
    if (!xstrcasecmp(reason, "AssocGrpUnknown")) return WAIT_ASSOC_GRP_UNK;
    ...
    if (!xstrcasecmp(reason, "AssocGrpEnergy")) return WAIT_ASSOC_GRP_ENERGY;
    ...
    if (!xstrcasecmp(reason, "AssocGrpGRES")) return WAIT_ASSOC_GRP_GRES;
    ...
    if (!xstrcasecmp(reason, "AssocGrpLicense")) return WAIT_ASSOC_GRP_LIC;
    ...
    if (!xstrcasecmp(reason, "AssocGrpBB")) return WAIT_ASSOC_GRP_BB;
    if (!xstrcasecmp(reason, "AssocGrpBBMinutes")) return WAIT_ASSOC_GRP_BB_MIN;
    ...
    if (!xstrcasecmp(reason, "QOSGrpUnknown")) return WAIT_QOS_GRP_UNK;
    ...
    if (!xstrcasecmp(reason, "QOSMinUnknown")) return WAIT_QOS_MIN_UNK;
    ...
    if (!xstrcasecmp(reason, "QOSMinMemory")) return WAIT_QOS_MIN_MEM;
    if (!xstrcasecmp(reason, "QOSGrpNodeMinutes")) return WAIT_QOS_GRP_NODE_MIN;
    ...
    if (!xstrcasecmp(reason, "QOSMinNode")) return WAIT_QOS_MIN_NODE;
    if (!xstrcasecmp(reason, "QOSGrpEnergy")) return WAIT_QOS_GRP_ENERGY;
    ...
    if (!xstrcasecmp(reason, "QOSMinEnergy")) return WAIT_QOS_MIN_ENERGY;
    if (!xstrcasecmp(reason, "QOSGrpGRES")) return WAIT_QOS_GRP_GRES;
    if (!xstrcasecmp(reason, "QOSGrpGRESMinutes")) return WAIT_QOS_GRP_GRES_MIN;
    ...
    if (!xstrcasecmp(reason, "QOSMinGRES")) return WAIT_QOS_MIN_GRES;
    if (!xstrcasecmp(reason, "QOSGrpLicense")) return WAIT_QOS_GRP_LIC;
    ...
    if (!xstrcasecmp(reason, "QOSMinLicense")) return WAIT_QOS_MIN_LIC;
    if (!xstrcasecmp(reason, "QOSGrpBB")) return WAIT_QOS_GRP_BB;
    if (!xstrcasecmp(reason, "QOSGrpBBMinutes")) return WAIT_QOS_GRP_BB_MIN;
    if (!xstrcasecmp(reason, "QOSGrpBBRunMinutes"))
        return WAIT_QOS_GRP_BB_RUN_MIN;
    if (!xstrcasecmp(reason, "QOSMaxBBPerJob")) return WAIT_QOS_MAX_BB_PER_JOB;
    ...
    if (!xstrcasecmp(reason, "QOSMinBB")) return WAIT_QOS_MIN_BB;
    if (!xstrcasecmp(reason, "DeadLine")) return FAIL_DEADLINE;
    ...
    if (!xstrcasecmp(reason, "PartitionConfig")) return WAIT_PART_CONFIG;
    if (!xstrcasecmp(reason, "AccountingPolicy")) return WAIT_ACCOUNT_POLICY;
    if (!xstrcasecmp(reason, "FedJobLock")) return WAIT_FED_JOB_LOCK;
    if (!xstrcasecmp(reason, "OutOfMemory")) return FAIL_OOM;
    if (!xstrcasecmp(reason, "MaxMemPerLimit")) return WAIT_PN_MEM_LIMIT;
    if (!xstrcasecmp(reason, "AssocGrpBilling")) return WAIT_ASSOC_GRP_BILLING;
    ...
    if (!xstrcasecmp(reason, "QOSGrpBilling")) return WAIT_QOS_GRP_BILLING;
    ...
    if (!xstrcasecmp(reason, "QOSMinBilling")) return WAIT_QOS_MIN_BILLING;
    if (!xstrcasecmp(reason, "ReservationDeleted")) return WAIT_RESV_DELETED;
    ...
    if (msg) {
        ...
        if (msg->kvs_comm_ptr) {
            for (i = 0; i < msg->kvs_comm_recs; i++) {
                if (!msg->kvs_comm_ptr[i]) continue;
...
extern const char *preempt_mode_string(uint16_t preempt_mode) {
    if (preempt_mode == PREEMPT_MODE_OFF) return "OFF";
    if (preempt_mode == PREEMPT_MODE_GANG) return "GANG";
    if (preempt_mode == PREEMPT_MODE_WITHIN) return "WITHIN";
...
extern uint16_t preempt_mode_num(const char *preempt_mode) {
    ...
    if (preempt_mode == NULL) return mode_num;
    ...
    if ((preempt_modes > 1) ||
        ((mode_num & PREEMPT_MODE_GANG) && (mode_num & PREEMPT_MODE_WITHIN))) {
        mode_num = NO_VAL16;
    }
...
extern uint16_t log_string2num(const char *name) {
    if (name == NULL) return NO_VAL16;

    if ((name[0] >= '0') && (name[0] <= '9')) return (uint16_t)atoi(name);

    if (!xstrcasecmp(name, "quiet")) return (uint16_t)0;
    if (!xstrcasecmp(name, "fatal")) return (uint16_t)1;
    if (!xstrcasecmp(name, "error")) return (uint16_t)2;
    if (!xstrcasecmp(name, "info")) return (uint16_t)3;
    if (!xstrcasecmp(name, "verbose")) return (uint16_t)4;
    if (!xstrcasecmp(name, "debug")) return (uint16_t)5;
    if (!xstrcasecmp(name, "debug2")) return (uint16_t)6;
    if (!xstrcasecmp(name, "debug3")) return (uint16_t)7;
    if (!xstrcasecmp(name, "debug4")) return (uint16_t)8;
    if (!xstrcasecmp(name, "debug5")) return (uint16_t)9;
...
extern char *job_state_string(uint32_t inx) {
    ...
    if (inx & JOB_COMPLETING) return "COMPLETING";
    if (inx & JOB_STAGE_OUT) return "STAGE_OUT";
    if (inx & JOB_CONFIGURING) return "CONFIGURING";
    if (inx & JOB_RESIZING) return "RESIZING";
    if (inx & JOB_REQUEUE) return "REQUEUED";
    if (inx & JOB_REQUEUE_FED) return "REQUEUE_FED";
    if (inx & JOB_REQUEUE_HOLD) return "REQUEUE_HOLD";
    if (inx & JOB_SPECIAL_EXIT) return "SPECIAL_EXIT";
    if (inx & JOB_STOPPED) return "STOPPED";
    if (inx & JOB_REVOKED) return "REVOKED";
    if (inx & JOB_RESV_DEL_HOLD) return "RESV_DEL_HOLD";
    if (inx & JOB_SIGNALING) return "SIGNALING";
...
extern char *job_state_string_compact(uint32_t inx) {
    /* Process JOB_STATE_FLAGS */
    if (inx & JOB_COMPLETING) return "CG";
    if (inx & JOB_STAGE_OUT) return "SO";
    if (inx & JOB_CONFIGURING) return "CF";
    if (inx & JOB_RESIZING) return "RS";
    if (inx & JOB_REQUEUE) return "RQ";
    if (inx & JOB_REQUEUE_FED) return "RF";
    if (inx & JOB_REQUEUE_HOLD) return "RH";
    if (inx & JOB_SPECIAL_EXIT) return "SE";
    if (inx & JOB_STOPPED) return "ST";
    if (inx & JOB_REVOKED) return "RV";
    if (inx & JOB_RESV_DEL_HOLD) return "RD";
    if (inx & JOB_SIGNALING) return "SI";
...
extern char *job_state_string_complete(uint32_t state) {
    ...
    /* Process JOB_STATE_FLAGS */
    if (state & JOB_LAUNCH_FAILED) xstrcat(state_str, ",LAUNCH_FAILED");
    if (state & JOB_UPDATE_DB) xstrcat(state_str, ",UPDATE_DB");
    if (state & JOB_COMPLETING) xstrcat(state_str, ",COMPLETING");
    if (state & JOB_CONFIGURING) xstrcat(state_str, ",CONFIGURING");
    if (state & JOB_POWER_UP_NODE) xstrcat(state_str, ",POWER_UP_NODE");
    if (state & JOB_RECONFIG_FAIL) xstrcat(state_str, ",RECONFIG_FAIL");
    if (state & JOB_RESIZING) xstrcat(state_str, ",RESIZING");
    if (state & JOB_REQUEUE) xstrcat(state_str, ",REQUEUED");
    if (state & JOB_REQUEUE_FED) xstrcat(state_str, ",REQUEUE_FED");
    if (state & JOB_REQUEUE_HOLD) xstrcat(state_str, ",REQUEUE_HOLD");
    if (state & JOB_SPECIAL_EXIT) xstrcat(state_str, ",SPECIAL_EXIT");
    if (state & JOB_STOPPED) xstrcat(state_str, ",STOPPED");
    if (state & JOB_REVOKED) xstrcat(state_str, ",REVOKED");
    if (state & JOB_RESV_DEL_HOLD) xstrcat(state_str, ",RESV_DEL_HOLD");
    if (state & JOB_SIGNALING) xstrcat(state_str, ",SIGNALING");
    if (state & JOB_STAGE_OUT) xstrcat(state_str, ",STAGE_OUT");
...
extern uint32_t job_state_num(const char *state_name) {
    ...
    for (i = 0; i < JOB_END; i++) {
        if (_job_name_test(i, state_name)) return i;
    }

    if (_job_name_test(JOB_COMPLETING, state_name)) return JOB_COMPLETING;
    if (_job_name_test(JOB_CONFIGURING, state_name)) return JOB_CONFIGURING;
    if (_job_name_test(JOB_RESIZING, state_name)) return JOB_RESIZING;
    if (_job_name_test(JOB_RESV_DEL_HOLD, state_name)) return JOB_RESV_DEL_HOLD;
    if (_job_name_test(JOB_REQUEUE, state_name)) return JOB_REQUEUE;
    if (_job_name_test(JOB_REQUEUE_FED, state_name)) return JOB_REQUEUE_FED;
    if (_job_name_test(JOB_REQUEUE_HOLD, state_name)) return JOB_REQUEUE_HOLD;
    if (_job_name_test(JOB_REVOKED, state_name)) return JOB_REVOKED;
    if (_job_name_test(JOB_SIGNALING, state_name)) return JOB_SIGNALING;
    if (_job_name_test(JOB_SPECIAL_EXIT, state_name)) return JOB_SPECIAL_EXIT;
    if (_job_name_test(JOB_STAGE_OUT, state_name)) return JOB_STAGE_OUT;
    if (_job_name_test(JOB_STOPPED, state_name)) return JOB_STOPPED;
...
extern char *health_check_node_state_str(uint32_t node_state) {
    ...
    if ((node_state & HEALTH_CHECK_NODE_ANY) == HEALTH_CHECK_NODE_ANY) {
        if (state_str[0]) xstrcat(state_str, ",");
...
extern char *reservation_flags_string(reserve_info_t *resv_ptr) {
    ...
    if (flags & RESERVE_FLAG_MAINT) xstrcat(flag_str, "MAINT");
    if (flags & RESERVE_FLAG_NO_MAINT) {
        if (flag_str[0]) xstrcat(flag_str, ",");
    ...
    if (flags & RESERVE_FLAG_PURGE_COMP) {
        ...
        if (resv_ptr->purge_comp_time) {
            char tmp_pct[40];
            secs2time_str(resv_ptr->purge_comp_time, tmp_pct, sizeof(tmp_pct));
...
extern char *bb_state_string(uint16_t state) {
    ...
    if (state == BB_STATE_PENDING) return "pending";
    if (state == BB_STATE_ALLOCATING) return "allocating";
    if (state == BB_STATE_ALLOCATED) return "allocated";
    if (state == BB_STATE_DELETING) return "deleting";
    if (state == BB_STATE_DELETED) return "deleted";
    if (state == BB_STATE_STAGING_IN) return "staging-in";
    if (state == BB_STATE_STAGED_IN) return "staged-in";
    if (state == BB_STATE_PRE_RUN) return "pre-run";
    if (state == BB_STATE_RUNNING) return "running";
    if (state == BB_STATE_SUSPEND) return "suspended";
    if (state == BB_STATE_POST_RUN) return "post-run";
    if (state == BB_STATE_STAGING_OUT) return "staging-out";
    if (state == BB_STATE_STAGED_OUT) return "staged-out";
    if (state == BB_STATE_TEARDOWN) return "teardown";
    if (state == BB_STATE_TEARDOWN_FAIL) return "teardown-fail";
    if (state == BB_STATE_COMPLETE) return "complete";
...
extern uint16_t bb_state_num(char *tok) {
    if (!xstrcasecmp(tok, "pending")) return BB_STATE_PENDING;
    if (!xstrcasecmp(tok, "allocating")) return BB_STATE_ALLOCATING;
    if (!xstrcasecmp(tok, "allocated")) return BB_STATE_ALLOCATED;
    if (!xstrcasecmp(tok, "deleting")) return BB_STATE_DELETING;
    if (!xstrcasecmp(tok, "deleted")) return BB_STATE_DELETED;
    if (!xstrcasecmp(tok, "staging-in")) return BB_STATE_STAGING_IN;
    if (!xstrcasecmp(tok, "staged-in")) return BB_STATE_STAGED_IN;
    if (!xstrcasecmp(tok, "pre-run")) return BB_STATE_PRE_RUN;
    if (!xstrcasecmp(tok, "running")) return BB_STATE_RUNNING;
    if (!xstrcasecmp(tok, "suspend")) return BB_STATE_SUSPEND;
    if (!xstrcasecmp(tok, "post-run")) return BB_STATE_POST_RUN;
    if (!xstrcasecmp(tok, "staging-out")) return BB_STATE_STAGING_OUT;
    if (!xstrcasecmp(tok, "staged-out")) return BB_STATE_STAGED_OUT;
    if (!xstrcasecmp(tok, "teardown")) return BB_STATE_TEARDOWN;
    if (!xstrcasecmp(tok, "teardown-fail")) return BB_STATE_TEARDOWN_FAIL;
    if (!xstrcasecmp(tok, "complete")) return BB_STATE_COMPLETE;
    return 0;
}
...
extern char *node_state_string(uint32_t inx) {
    ...
    if (inx & NODE_STATE_INVALID_REG) return "INVAL";
    ...
    if (drain_flag) {
        if (comp_flag || (base == NODE_STATE_ALLOCATED) ||
            ...
            if (maint_flag) return "DRAINING$";
            if (reboot_issued_flag) return "DRAINING^";
            if (reboot_flag) return "DRAINING@";
            if (power_up_flag) return "DRAINING#";
            if (powering_down_flag) return "DRAINING%";
            if (powered_down_flag) return "DRAINING~";
            if (no_resp_flag) return "DRAINING*";
            return "DRAINING";
        } else {
            if (maint_flag) return "DRAINED$";
            if (reboot_issued_flag) return "DRAINED^";
            if (reboot_flag) return "DRAINED@";
            if (power_up_flag) return "DRAINED#";
            if (powering_down_flag) return "DRAINED%";
            if (powered_down_flag) return "DRAINED~";
            if (no_resp_flag) return "DRAINED*";
            return "DRAINED";
        }
    }
    if (fail_flag) {
        if (comp_flag || (base == NODE_STATE_ALLOCATED)) {
            if (no_resp_flag) return "FAILING*";
            return "FAILING";
        } else {
            if (no_resp_flag) return "FAIL*";
            return "FAIL";
        }
    }

    if (inx == NODE_STATE_REBOOT_ISSUED) return "REBOOT^";
    if (inx == NODE_STATE_REBOOT_CANCEL) return "CANCEL_REBOOT";
    if (inx == NODE_STATE_CLOUD) return "CLOUD";
    if (inx == NODE_STATE_MAN_POWER_DOWN) return "MAN_POWER_DOWN";
    if (inx == NODE_STATE_MAN_POWER_UP) return "MAN_POWER_UP";
    if (inx == NODE_STATE_POWERING_DOWN) return "POWERING_DOWN";
    if (inx == NODE_STATE_POWER_SAVE) return "POWER_DOWN";
    if (inx == NODE_STATE_POWER_UP) return "POWER_UP";
    if (base == NODE_STATE_DOWN) {
        if (maint_flag) return "DOWN$";
        if (reboot_issued_flag) return "DOWN^";
        if (reboot_flag) return "DOWN@";
        if (power_up_flag) return "DOWN#";
        if (powering_down_flag) return "DOWN%";
        if (powered_down_flag) return "DOWN~";
        if (no_resp_flag) return "DOWN*";
        return "DOWN";
    }

    if (base == NODE_STATE_ALLOCATED) {
        if (maint_flag) return "ALLOCATED$";
        if (reboot_issued_flag) return "ALLOCATED^";
        if (reboot_flag) return "ALLOCATED@";
        if (power_up_flag) return "ALLOCATED#";
        if (powering_down_flag) return "ALLOCATED%";
        if (powered_down_flag) return "ALLOCATED~";
        if (no_resp_flag) return "ALLOCATED*";
        if (comp_flag) return "ALLOCATED+";
        return "ALLOCATED";
    }
    if (comp_flag) {
        if (maint_flag) return "COMPLETING$";
        if (reboot_issued_flag) return "COMPLETING^";
        if (reboot_flag) return "COMPLETING@";
        if (power_up_flag) return "COMPLETING#";
        if (powering_down_flag) return "COMPLETING%";
        if (powered_down_flag) return "COMPLETING~";
        if (no_resp_flag) return "COMPLETING*";
        return "COMPLETING";
    }
    if (base == NODE_STATE_IDLE) {
        if (maint_flag) return "IDLE$";
        if (reboot_issued_flag) return "IDLE^";
        if (reboot_flag) return "IDLE@";
        if (power_up_flag) return "IDLE#";
        if (powering_down_flag) return "IDLE%";
        if (powered_down_flag) return "IDLE~";
        if (no_resp_flag) return "IDLE*";
        if (net_flag) return "PERFCTRS";
        if (res_flag) return "RESERVED";
        if (planned_flag) return "PLANNED";
        return "IDLE";
    }
    if (base == NODE_STATE_MIXED) {
        if (maint_flag) return "MIXED$";
        if (reboot_issued_flag) return "MIXED^";
        if (reboot_flag) return "MIXED@";
        if (power_up_flag) return "MIXED#";
        if (powering_down_flag) return "MIXED%";
        if (powered_down_flag) return "MIXED~";
        if (no_resp_flag) return "MIXED*";
        if (planned_flag) return "MIXED-";
        return "MIXED";
    }
    if (base == NODE_STATE_FUTURE) {
        if (maint_flag) return "FUTURE$";
        if (reboot_issued_flag) return "FUTURE^";
        if (reboot_flag) return "FUTURE@";
        if (power_up_flag) return "FUTURE#";
        if (powering_down_flag) return "FUTURE%";
        if (powered_down_flag) return "FUTURE~";
        if (no_resp_flag) return "FUTURE*";
        return "FUTURE";
    }
    if (resume_flag) return "RESUME";
    if (base == NODE_STATE_UNKNOWN) {
        if (no_resp_flag) return "UNKNOWN*";
        return "UNKNOWN";
    }
    return "?";
}
...
extern char *node_state_string_compact(uint32_t inx) {
    ...
    if (maint_flag) {
        if (drain_flag || (inx == NODE_STATE_ALLOCATED) ||
            (inx == NODE_STATE_DOWN) || (inx == NODE_STATE_MIXED))
            ;
        else if (no_resp_flag)
            return "MAINT*";
        else
    ...
    if (drain_flag) {
        if (comp_flag || (inx == NODE_STATE_ALLOCATED) ||
            ...
            if (maint_flag) return "DRNG$";
            if (reboot_issued_flag) return "DRNG^";
            if (reboot_flag) return "DRNG@";
            if (power_up_flag) return "DRNG#";
            if (powering_down_flag) return "DRNG%";
            if (powered_down_flag) return "DRNG~";
            if (no_resp_flag) return "DRNG*";
            return "DRNG";
        } else {
            if (maint_flag) return "DRAIN$";
            if (reboot_issued_flag) return "DRAIN^";
            if (reboot_flag) return "DRAIN@";
            if (power_up_flag) return "DRAIN#";
            if (powering_down_flag) return "DRAIN%";
            if (powered_down_flag) return "DRAIN~";
            if (no_resp_flag) return "DRAIN*";
            return "DRAIN";
        }
    }
    if (fail_flag) {
        if (comp_flag || (inx == NODE_STATE_ALLOCATED)) {
            if (no_resp_flag) return "FAILG*";
            return "FAILG";
        } else {
            if (no_resp_flag) return "FAIL*";
            return "FAIL";
    ...
    if (inx == NODE_STATE_REBOOT_ISSUED) return "BOOT^";
    if (inx == NODE_STATE_REBOOT_CANCEL) return "CANC_R";
    if (inx == NODE_STATE_CLOUD) return "CLOUD";
    if (inx == NODE_STATE_MAN_POWER_DOWN) return "MAN_POW_DN";
    if (inx == NODE_STATE_MAN_POWER_UP) return "MAN_POW_UP";
    if (inx == NODE_STATE_POWERING_DOWN) return "POWRNG_DN";
    if (inx == NODE_STATE_POWER_SAVE) return "POW_DN";
    if (inx == NODE_STATE_POWER_UP) return "POW_UP";
    ...
    if (inx == NODE_STATE_ALLOCATED) {
        if (maint_flag) return "ALLOC$";
        if (reboot_issued_flag) return "ALLOC^";
        if (reboot_flag) return "ALLOC@";
        if (power_up_flag) return "ALLOC#";
        if (powering_down_flag) return "ALLOC%";
        if (powered_down_flag) return "ALLOC~";
        if (no_resp_flag) return "ALLOC*";
        if (comp_flag) return "ALLOC+";
        return "ALLOC";
    }
    if (comp_flag) {
        if (maint_flag) return "COMP$";
        if (reboot_issued_flag) return "COMP^";
        if (reboot_flag) return "COMP@";
        if (power_up_flag) return "COMP#";
        if (powering_down_flag) return "COMP%";
        if (powered_down_flag) return "COMP~";
        if (no_resp_flag) return "COMP*";
        return "COMP";
    }
    if (inx == NODE_STATE_IDLE) {
        ...
        if (powered_down_flag) return "IDLE~";
        if (no_resp_flag) return "IDLE*";
        if (net_flag) return "NPC";
        if (res_flag) return "RESV";
        if (planned_flag) return "PLND";
        return "IDLE";
    }
    if (inx == NODE_STATE_MIXED) {
        if (maint_flag) return "MIX$";
        if (reboot_issued_flag) return "MIX^";
        if (reboot_flag) return "MIX@";
        if (power_up_flag) return "MIX#";
        if (powering_down_flag) return "MIX%";
        if (powered_down_flag) return "MIX~";
        if (no_resp_flag) return "MIX*";
        if (planned_flag) return "MIX-";
        return "MIX";
    }
    if (inx == NODE_STATE_FUTURE) {
        if (maint_flag) return "FUTR$";
        if (reboot_issued_flag) return "FUTR^";
        if (reboot_flag) return "FUTR@";
        if (power_up_flag) return "FUTR#";
        if (powering_down_flag) return "FUTR%";
        if (powered_down_flag) return "FUTR~";
        if (no_resp_flag) return "FUTR*";
        return "FUTR";
    }
    if (resume_flag) return "RESM";
    if (inx == NODE_STATE_UNKNOWN) {
        if (no_resp_flag) return "UNK*";
        return "UNK";
    }
    return "?";
}
...
extern char *power_flags_str(uint16_t power_flags) {
    if (power_flags & SLURM_POWER_FLAGS_LEVEL) return "LEVEL";
    return "";
}
...
extern void private_data_string(uint16_t private_data, char *str, int str_len) {
    ...
    if (str[0] == '\0') strcat(str, "none");
}

extern void accounting_enforce_string(uint16_t enforce, char *str,
                                      int str_len) {
    if (str_len > 0) str[0] = '\0';
    if (str_len < 30) {
...
extern char *cray_nodelist2nids(hostlist_t hl_in, char *nodelist) {
    ...
    while ((node_name = hostlist_shift(hl))) {
        for (i = 0; node_name[i]; i++) {
            if (!isdigit(node_name[i])) continue;
...
static void _free_all_job_info(job_info_msg_t *msg) {
    ...
    if ((msg == NULL) || (msg->job_array == NULL)) return;
...
static void _free_all_step_info(job_step_info_response_msg_t *msg) {
    ...
    if ((msg == NULL) || (msg->job_steps == NULL)) return;
...
static void _free_all_front_end_info(front_end_info_msg_t *msg) {
    ...
    if ((msg == NULL) || (msg->front_end_array == NULL)) return;
...
extern void slurm_init_node_info_t(node_info_t *msg, bool clear) {
    ...
    if (clear) memset(msg, 0, sizeof(node_info_t));
...
static void _free_all_node_info(node_info_msg_t *msg) {
    ...
    if ((msg == NULL) || (msg->node_array == NULL)) return;
...
static void _free_all_partitions(partition_info_msg_t *msg) {
    ...
    if ((msg == NULL) || (msg->partition_array == NULL)) return;
...
static void _free_all_reservations(reserve_info_msg_t *msg) {
    ...
    if ((msg == NULL) || (msg->reservation_array == NULL)) return;
...
extern void slurm_free_shares_response_msg(shares_response_msg_t *msg) {
    if (msg) {
        int i;
        if (msg->tres_names) {
            for (i = 0; i < msg->tres_cnt; i++) xfree(msg->tres_names[i]);
...
extern int slurm_free_msg_data(slurm_msg_type_t type, void *data) {
    if (!data) return SLURM_SUCCESS;

    /* this message was never loaded */
    if ((uint16_t)type == NO_VAL16) return SLURM_SUCCESS;
...
extern bool valid_spank_job_env(char **spank_job_env,
    ...
    for (i = 0; i < spank_job_env_size; i++) {
        if (!xstrncmp(spank_job_env[i], "SPANK_", 6)) continue;
...
extern void slurm_free_license_info_request_msg(
    license_info_request_msg_t *msg) {
    xfree(msg);
}
...
extern int parse_part_enforce_type(char *enforce_part_type, uint16_t *param) {
    ...
    if (!xstrcasecmp(value, "yes") || !xstrcasecmp(value, "up") ||
        !xstrcasecmp(value, "true") || !xstrcasecmp(value, "1") ||
        !xstrcasecmp(value, "any")) {
        *param = PARTITION_ENFORCE_ANY;
    } else if (!xstrcasecmp(value, "no") || !xstrcasecmp(value, "down") ||
               !xstrcasecmp(value, "false") || !xstrcasecmp(value, "0")) {
...
extern bool verify_step_id(slurm_step_id_t *object, slurm_step_id_t *key) {
    if (key->job_id != object->job_id) return 0;

    /* Any step will do */
    if (key->step_id == NO_VAL) return 1;
...
extern char *slurm_get_selected_step_id(char *job_id_str, int len,
                                        slurm_selected_step_t *selected_step) {
    int pos = 0;

    pos = snprintf(job_id_str, len, "%u", selected_step->step_id.job_id);
    if (pos > len) goto endit;
    ...
    if (selected_step->step_id.step_id != NO_VAL) {
        ...
        log_build_step_id_str(&selected_step->step_id, job_id_str + pos,
                              len - pos,
...
extern void xlate_array_task_str(char **array_task_str,
    ...
    if (!array_task_str || !*array_task_str || !*array_task_str[0]) {
        if (array_bitmap) *array_bitmap = NULL;
        return;
    }
    ...
    if (bit_unfmt_hexmask(task_bitmap, *array_task_str) == -1)
        error("%s: bit_unfmt_hexmask error on '%s'", __func__, *array_task_str);

    if (array_bitmap) *array_bitmap = (void *)task_bitmap;
    ...
    if (((i_last - i_first) > 10) && (bit_set_count(task_bitmap) > 5) &&
        ...
        for (i = i_first + 1; i <= i_last; i++) {
            if (!bit_test(task_bitmap, i)) continue;
            if (i_step == 0) {
                i_step = i - i_prev;
    ...
    if (bitstr_len == -1) {
        ...
        if (bitstr_len_str) bitstr_len = atoi(bitstr_len_str);
        if (bitstr_len < 0)
            bitstr_len = 64;
        else
    ...
    if (bitstr_len > 0) {
        ...
        len = strlen(out_buf);
        if (len > (buf_size - 3)) {
            for (i = 0; i < 3; i++) out_buf[buf_size - 2 - i] = '.';
        }
    } else {
...
out:
    if (array_max_tasks) xstrfmtcat(out_buf, "%%%u", array_max_tasks);

    xfree(*array_task_str);
    *array_task_str = out_buf;

    if (!array_bitmap) bit_free(task_bitmap);
}
